<?php
    ...
    function msgQueryProfile( $aRequest )
    {
        if ( validUser() )
        {
            ...
            $Users = $Connector->prepare( 'SELECT Login, UNIX_TIMESTAMP(Created) AS CreatedUTC, ExternalBinding, BindingActive '.
            ...
            $AttendanceQuery = $Connector->prepare(  'SELECT `Status`, `Role`, COUNT(RaidId) AS `Count` '.
                ...
                'LEFT JOIN `'.RP_TABLE_PREFIX.'Location` USING(LocationId) '.
                'WHERE `'.RP_TABLE_PREFIX.'Attendance`.UserId = :UserId AND Start > FROM_UNIXTIME(:Created) AND Start < FROM_UNIXTIME(:Now) AND Game = :Game '.
