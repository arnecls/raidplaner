<?php
    ...
    {
        if ( validRaidlead() || (validPrivileged() && isset($aRequest['type']) && $aRequest['type'] == 'event') )
        {
            global $gGame;
            ...
            if ( $LocationId != 0 )
            {
                ...
                $UserSettingsQuery = $Connector->prepare('SELECT UserId, Name, IntValue, TextValue '.
                   'FROM `'.RP_TABLE_PREFIX.'UserSetting` '.
                ...
                for ($rc=0; $rc<$Repeat; ++$rc)
                {
                    $NewRaidQuery = $Connector->prepare('INSERT INTO `'.RP_TABLE_PREFIX.'Raid` '.
                                                        '(LocationId, UserId, Type, Size, Start, End, Mode, Description, SlotRoles, SlotCount ) '.
                                                        'VALUES (:LocationId, :UserId, :Type, :Size, FROM_UNIXTIME(:Start), FROM_UNIXTIME(:End), :Mode, :Description, '.
                                                        ':SlotRoles, :SlotCount)');
                    ...
                    $NewRaidQuery->bindValue(':Type',        $aRequest['type'], PDO::PARAM_STR);
