...
function msgRaidUpdate( $Request )
{
    if ( ValidRaidlead() )
    {
        ...
        $UpdateRaidSt->bindValue(":Mode",        $Request["mode"], PDO::PARAM_INT);
        $UpdateRaidSt->bindValue(":Description", requestToXML( $Request["description"], ENT_COMPAT, "UTF-8" ), PDO::PARAM_STR);
        
        $SlotSizes = Array(
            intval($Request["slotsRole1"]), intval($Request["slotsRole2"]), intval($Request["slotsRole3"]),
            intval($Request["slotsRole4"]), intval($Request["slotsRole5"])
        );
