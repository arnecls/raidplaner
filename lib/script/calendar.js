...
function generateRaidTooltip( aRaidDataXML, aDateIsInThePast, aCalendarView )
{
    ...
    var startTime = aRaidDataXML.children("start:first").text();
    var endTime   = aRaidDataXML.children("end:first").text();
    var startDate = aRaidDataXML.children("startDate:first").text();
    var endDate   = aRaidDataXML.children("endDate:first").text();
    ...
    HTMLString += "<div class=\"time\">" + formatTimeStringUTC(startDate,startTime) + " - " + formatTimeStringUTC(endDate,endTime) + "</div>";
