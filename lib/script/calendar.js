...
function generateRaidTooltip( aRaid, aDateIsInThePast, aCalendarView )
{
    ...
    HTMLString += "<div class=\"icon\" onclick=\"changeContext('raid,setup," + RaidId + "')\" style=\"background: url('images/icons/"+gSite.Iconset+"/raidbig/" + RaidImg + "')\"></div>";
    ...
    for (var WeekIdx=0; WeekIdx<6; ++WeekIdx)
    {
        ...
        for (var ColIdx=0; ColIdx<7; ++ColIdx)
        {
                    ...
                    matchesDate(aXHR.raid[RaidIdx], TimeIter))
            {
                ...
                HTMLString += "<span id=\"raid" + RaidId + "\" class=\"raid\" style=\"background: url('images/icons/"+gSite.Iconset+"/raidsmall/" + RaidImg + "')\" class=\"raidimg\">";
