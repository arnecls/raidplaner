...
function generateRaidTooltip( aRaid, aDateIsInThePast, aCalendarView )
{
    ...
    var AttendedCharId   = aRaid.attendanceIndex;
    var RaidId   = aRaid.id;
    ...
    HTMLString += "<div class=\"location"+((aRaid.type =="event") ? " event": "")+"\" style=\"font-size: "+NameFontSize+"px\">" + RaidName + ((aRaid.mode != "all") ? " ("+RaidSize+")" : "") + "</div>";
    ...
    HTMLString += "<div class=\"attends\">" + aRaid.attended + " / " + RaidSize + " " + L("Players") + "</div>";
        ...
        HTMLString += "<div class=\"attends\">" + aRaid.attended + " " + L("Attendants") + "</div>";
    }
    ...
    if ( !aDateIsInThePast && HasCharacters )
    {
        ...
        for ( var i=0; i<gUser.characterIds.length; ++i )
        {
            if (gUser.characterGames[i] == gGame.GameId)
            {
                var Selected = (AttendedCharId == gUser.characterIds[i]);
                if (Selected)
                {
                    CharacterIdx = i;
                }

                HTMLString += "<option value=\"" + gUser.characterIds[i] + "\"" + ((Selected) ? " selected" : "") + ">" + gUser.characterNames[i] + "</option>";
            }
        }

        HTMLString += "<option value=\"-1\"" + ((AttendedCharId == -1) ? " selected" : "") + ">" + L("Absent") + "</option>";
        HTMLString += "</select>";

        // Show roles/classes for character

        if ( !gSite.PrimaryRole &&
            ((StatusValue == "available") || (aRaid.mode == "attend") || (aRaid.mode == "all")) &&
            (CharacterIdx >= 0) )
        {
            if ( gGame.ClassMode == "multi" )
            {
                var NumClasses = gUser.characterClass[CharacterIdx].length;
                if (NumClasses > 1)
                {
                    HTMLString += "<select class=\"attend_sub\">";
                    for (var i=0; i<NumClasses; ++i)
                    {
                        var ClassId = gUser.characterClass[CharacterIdx][i];
                        var Selected = ClassId == aRaid.classId;
                        HTMLString += "<option value=\""+i+"\"" + ((Selected) ? " selected" : "") + ">"+L(gGame.Classes[ClassId].name)+"</option>";
                    }
            ...
            {
                var Role1Id = gUser.role1[CharacterIdx];
                var Role2Id = gUser.role2[CharacterIdx];

                if (Role1Id != Role2Id)
                {
                    HTMLString += "<select class=\"attend_sub\">";
                    HTMLString += "<option value=\"0\"" + ((aRaid.role == Role1Id) ? " selected" : "") + ">"+L(gGame.Roles[Role1Id].name)+"</option>";
                    HTMLString += "<option value=\"1\"" + ((aRaid.role == Role2Id) ? " selected" : "") + ">"+L(gGame.Roles[Role2Id].name)+"</option>";
                    HTMLString += "</select>";
                }
            }
        }
...
function generateCalendar(aXHR)
{
    ...
    var CalendarBigIcons = (aXHR.showBigIcons);

    var HTMLString = "<div id=\"calendarPositioner\"><table id=\"calendar\" cellspacing=\"0\" border=\"0\"" + (CalendarBigIcons ? ' class="bigicons"' : '') + ">";
    ...
    for (var WeekIdx=0; WeekIdx<6; ++WeekIdx)
    {
        ...
        for (var ColIdx=0; ColIdx<7; ++ColIdx)
        {
                    ...
                    matchesDate(aXHR.raid[RaidIdx], TimeIter))
            {
                ...
                var RoleClass = (RaidRole.length > 0 ? ' ' + RaidRole : '');

                HTMLString += "<span id=\"raid" + RaidId + "\" index=\""+RaidIdx+"\" locked=\""+(DateIsInThePast || RaidIsLocked)+"\" class=\"raid clickable\" style=\"background: url('themes/icons/"+gSite.Iconset+"/"+(CalendarBigIcons ? 'raidbig' : 'raidsmall') + "/" + RaidImg + "')\" class=\"raidimg\">";

                if (CurrentRaid.type == "raid") {
                    HTMLString += "<div class=\""+ StatusClass + RoleClass + " shadow_inlay\"><span class=\"raidsize\">" + RaidSize + "</span></div>";
                } else {
                    HTMLString += "<div class=\""+ StatusClass + RoleClass + " shadow_inlay\">&bull;&bull;&bull;</div>";
                }

                if (CalendarBigIcons)
                {
                    HTMLString += "<span class=\"role_info "+RoleClass+"\"></span>";
                }
