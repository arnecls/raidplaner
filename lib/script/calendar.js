...
function DisplayRaidTooltip( a_RaidDataXML, a_DateIsInThePast, a_CalendarView )
{
    ...
    var Role        = g_RoleIdents[parseInt(a_RaidDataXML.children("role").text())];
    ...
    HTMLString += "<div class=\"icon\" onclick=\"changeContext('raid," + raidId + "')\" style=\"background: url('images/raidbig/" + raidImg + "')\"></div>";
...
function displayCalendar( a_XMLData )
{
    ...
    var ActiveMonth = parseInt( Message.children("displayMonth").text(), 10 );
    ...
    $(".raid")
        ...
        .dblclick( function( event ) { changeContext( "raid," + parseInt( $(this).attr("id").substr(4) ) ); event.stopPropagation(); } );
