...
function hash( a_Key, a_HashType, a_Password, a_Salt, a_OnUpdate, a_OnDone ) 
{
    ...
    case "jml_md5s":
        HashedPassword = hash_jml_md5s(a_Password, a_Salt);
        break;
        
    case "vanilla_md5r":
        hash_vanilla_md5r(a_Password, a_Salt, a_OnUpdate, function(HashedPassword) {
            a_OnDone( finalHash(a_Key, HashedPassword) );
        });
        return;
...
function hash_jml_md5s(a_Password, a_Salt)
{
    return CryptoJS.MD5(a_Password + a_Salt);
}

// -----------------------------------------------------------------------------

function hash_vanilla_md5r(a_Password, a_Salt, a_OnUpdate, a_OnDone)
{
    var parts   = a_Salt.split(":");
    var countB2 = parseInt(parts[0], 10);
    var count   = 1 << countB2;
    ...
    var loopFunc = function() 
    {
        ...
        a_OnUpdate(100);
        a_OnDone("$P$" + s_Itoa64.charAt(countB2) + salt + encode64(hash.words));
    };
