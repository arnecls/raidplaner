...
function hash( aKey, aHashType, aPassword, aSalt, aOnUpdate, aOnDone )
{
    ...
    case "jml_bf":
    case "phpbb3_bf":
        HashedPassword = hash_jml_bf(aPassword, aSalt, aOnUpdate, function(HashedPassword) {
            aOnDone( finalHash(aKey, HashedPassword) );
        });
        return;
    ...
    case "wbb_bf":
        HashedPassword = hash_wbb_bf(aPassword, aSalt, aOnUpdate, function(HashedPassword) {
            aOnDone( finalHash(aKey, HashedPassword) );
        });
        return;

    case "bt_bf":
        HashedPassword = hash_bt_bf(aPassword, aSalt, aOnUpdate, function(HashedPassword) {
            aOnDone( finalHash(aKey, HashedPassword) );
        });
        return;
...
{
    var saltWords = CryptoJS.enc.Hex.parse(aSalt);
    var passWords = CryptoJS.enc.Utf8.parse(aPassword);

    return CryptoJS.SHA1(saltWords.concat(passWords));
}
...
{
    return CryptoJS.MD5(aPassword + aSalt).toString() + ":" + aSalt;
}

// -----------------------------------------------------------------------------

function hash_jml_bf(aPassword, aSalt, aOnUpdate, aOnDone)
{
    var Bcrypt = new bCrypt();

    // bCrypt does not know $2y$, so we're cheating here
    var Method = aSalt.substr(0,3);
    var FixedSalt = "$2a" + aSalt.substr(3);

    Bcrypt.hashpw( aPassword, FixedSalt, aOnUpdate, function(aBfHash) {
        aOnUpdate(100);
        var restored = Method + aBfHash.substr(3);
        aOnDone(restored);
    });
}
...
function hash_wbb_bf(aPassword, aSalt, aOnUpdate, aOnDone)
{
    var Bcrypt = new bCrypt();
    ...
    Bcrypt.hashpw( aPassword, FixedSalt, function(aProgress) { aOnUpdate(aProgress / 2); }, function(aBfHash) {
        aOnUpdate(100);

        Bcrypt.hashpw( aBfHash, FixedSalt, function(aProgress) { aOnUpdate(50 + aProgress/2); }, function(aBfHash) {
            aOnUpdate(200);
            var restored = Method + aBfHash.substr(3);
            aOnDone(restored);
        });
...
function hash_bt_bf(aPassword, aSalt, aOnUpdate, aOnDone)
{
    var Bcrypt = new bCrypt();

    Bcrypt.hashpw( aPassword, aSalt, aOnUpdate, function(aBfHash) {
        aOnUpdate(100);
        var restored = "$2a" + aBfHash.substr(3);
        aOnDone(restored);
    });
}