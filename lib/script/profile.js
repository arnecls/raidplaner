    ...
    this.generateSlots = function( aIndex, aGrpIdx, aMaxSlots )
    {
        ...
        for ( var i=0; (CharIdx < this.mCharacters.length) && (i < aMaxSlots); ++i )
        {
            ...
            else
            {
                ...
                var ClassStyle = (CurrentChar.charClass[0] != "empty")
                    ? gGame.Classes[CurrentChar.charClass[0]].style
                    : "empty";
                
                HTMLString += "<div class=\"class\" style=\"top: 15px; left: 29px; background: url(themes/icons/"+gSite.Iconset+"/classesbig/" + ClassStyle + ".png)\">";
        ...
        Groups.each( function() {
            ...
            $(this).children(".charslot").each( function() {
                ...
                var onChangeClass = function() {
                    ...
                    {        
                        var classInfo = gGame.Classes[CurrentChar.charClass[0]];
                        var roleCount = classInfo.roles.length;
                        
                        if ( gGame.ClassMode == "multi" )
                        {
                            var defaultRole = classInfo.defaultRole;
                            
                            HTMLString += "<div class=\"single\" style=\"background-image: url(lib/layout/images/"+gGame.Roles[defaultRole].style+"_rnd.png)\"></div>";
                        }
                        else if ( roleCount == 1 )
                        {
                            HTMLString += "<div class=\"single\" style=\"background-image: url(lib/layout/images/"+gGame.Roles[CurrentChar.role1].style+"_rnd.png)\"></div>";
                        }        
                        else
                        {
                            HTMLString += "<div class=\"primary clickable\" style=\"background-image: url(lib/layout/images/"+gGame.Roles[CurrentChar.role1].style+"_rnd.png)\"></div>";
                            HTMLString += "<div class=\"secondary clickable\" style=\"background-image: url(lib/layout/images/"+gGame.Roles[CurrentChar.role2].style+"_rnd.png)\"></div>";
                        }
                    }
                    ...
                    $(".primary", CharSlot).click( function( aEvent ) {
                        ...
                        var classInfo = gGame.Classes[CurrentChar.charClass[0]];
                        var roles = classInfo.roles;
                        
                        var nextRoleIdx = (roles.indexOf(CurrentChar.role1) + 1) % roles.length;
                        CurrentChar.role1 = roles[nextRoleIdx];
                        
                        $(this).animate({left: -60}, 100, function() {
                            $(this).css("background-image","url(lib/layout/images/"+gGame.Roles[CurrentChar.role1].style+"_rnd.png)");
                    ...
                    $(".secondary", CharSlot).click( function( aEvent ) {
                        ...
                        var nextRoleIdx = (roles.indexOf(CurrentChar.role2) + 1) % roles.length;
                        CurrentChar.role2 = roles[nextRoleIdx];
                        
                        $(this).animate({right: -60}, 100, function() {
                            $(this).css("background-image","url(lib/layout/images/"+gGame.Roles[CurrentChar.role2].style+"_rnd.png)");
                ...
                if ( IsNew || (gGame.ClassMode == "multi"))
                {
...
function generateUserAttendance( aXHR )
{
    ...
    $.each(gGame.Roles, function(aRoleId, aRole) {
        var attendsForRole = aXHR.attendance.roles[aRoleId];
    
        RoleAttends[aRoleId] = (attendsForRole == undefined) ? 0 : attendsForRole;        
        RoleBarSizes[aRoleId] = (RoleAttends[aRoleId] / NumOk) * BarSize - Padding;
        NumRoleAttends += RoleAttends[aRoleId];
    });
    ...
    {
        $.each(gGame.Roles, function(aRoleId, aRole) {
           if ( RoleAttends[aRoleId] > 0 )
               HTMLString += "<span class=\"bar_part gradient_"+aRole.style+"\" style=\"width: " + RoleBarSizes[aRoleId].toFixed() + "px\">" + RoleAttends[aRoleId] + "</span>";
        });
    }
    ...
    $.each(gGame.Roles, function(aRoleId, aRole) {
       HTMLString += "<span>";
       HTMLString += "<div class=\"box gradient_"+aRole.style+"\"></div>";
       HTMLString += "<div class=\"label\">" + RoleAttends[aRoleId] + "x " + L(aRole.name) + "</div>";
       HTMLString += "</span>";
    });
...
function generateProfileSettings(aXHR)
{
    ...
    HTMLString += "<div id=\"profile_strprogress\"><span class=\"pglabel\">"+L("PassStrength")+"</span><span id=\"strength\" class=\"progress\"></span></div>";
    HTMLString += "</div></div>";
    ...
    HTMLString += "<div class=\"settingLine settingNewSection\"><div class=\"settingLabel\">"+L("AutoAttend")+"</div>";
    HTMLString += "<div class=\"settingField\">";
    HTMLString += "<input type=\"checkbox\" id=\"auto_attend\" value=\"true\""+((aXHR.settings.AutoAttend != undefined) ? " checked" : "")+"/>";
    HTMLString += "</div></div>";

    // Vacation

    HTMLString += "<div class=\"settingLine\"><div class=\"settingLabel\">"+L("VacationStart")+"</div>";
    HTMLString += "<div class=\"settingField\">";
...
function updateNewPasswordProgress( aProgress )
{
    ...
    else
    {
        if ( $("#profile_hashing").length === 0 )
        {
            ...
            $("#applyButton").before("<div id=\"profile_hashing\" style=\"right: "+offsetRight+"px\"><span class=\"pglabel\">"+L("HashingInProgress")+"</span><span id=\"hashprogress\" class=\"progress\"></span></div>");
        }
...
function triggerProfileUpdateData( aUserId, aEncodedOldPass )
{
    ...
    if (vacationEnd != null)
    {
        vacationEnd.setHours(23);
        vacationEnd.setMinutes(59);
    }
    ...
    var Parameters = {
        ...
        autoAttend      : $("#auto_attend").prop("checked"),
