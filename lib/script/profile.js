...
function CCharacterList()
{
    ...
    this.toggleMainChar = function( a_Idx )
    {
        ...
        {
            this.mCharacters[i].mainChar = (i == a_Idx);
        }
    };
    ...
    {
        this.mCharacters.splice(a_Idx,1);
        this.rebuildSlots();
    };
    ...
    this.displaySlots = function( a_Index, a_GrpIdx, a_MaxSlots )
    {
        ...
        for ( var i=0; (charIdx < this.mCharacters.length) && (i < a_MaxSlots); ++i )
        {
...
function renderUserAttendance( a_Message )
{
    // Static attend values

    var numRaids   = parseInt( a_Message.children("attendance").children("raids").text(), 10 );
    var numOk      = parseInt( a_Message.children("attendance").children("ok").text(), 10 );
    var numAvail   = parseInt( a_Message.children("attendance").children("available").text(), 10 );
    var numUnavail = parseInt( a_Message.children("attendance").children("unavailable").text(), 10 );
    ...
    {
       roleAttends[i]  = parseInt( a_Message.children("attendance").children(g_RoleIdents[i]).text(), 10 );
...
function renderProfile( a_XMLData )
{
...
function renderForeignProfile( a_XMLData )
{
    var Message = $(a_XMLData).children("messagehub");
...
function renderChangePassword( a_UserId )
{
    if ( gUser == null )
        return;
        
    var HTMLString = "<div class=\"login\" style=\"margin-top:-80px\">";    
    var OldPassText = (a_UserId == gUser.id) ? L("OldPassword") : L("AdminPassword");
        ...
        hash( key, method, pass, salt, updateNewPasswordProgress, function(encodedPass) {
            ...
            if ( parameters.id != gUser.id )
            {
                asyncQuery( "change_password", parameters, function(a_XMLData) {
                    if (renderForeignProfile(a_XMLData))
