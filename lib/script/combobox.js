(function( $ ) {
    $.widget( "ui.combobox", {
        ...
        _create: function() {
            ...
            var NodeId = (SelectNode.attr("id") == undefined) ? (new Date()).getTime() : SelectNode.attr("id");
            var Width  = SelectNode.outerWidth(true);
            var Height = Math.max(20, SelectNode.height());
            
            if ( this.options.icons.length > 0 )
            {
                if (this.options.iconAsValue)
                    Width = ButtonWidth + IconWidth + 11;
                else
                    Width += IconWidth;
            }
            ...
            $("#" + NodeId + "_open, #"+NodeId+"_value").click( function( aEvent ) {
                ...
                var maxWidth = (Widget.options.iconAsValue) 
                    ? 0 
                    : $("#" + NodeId + "_combobox").width();                
                
                OptionFields.each( function() {
                    if (Widget.options.iconAsValue)
                        maxWidth = Math.max($(this).outerWidth() + IconWidth, maxWidth);
                    else
            ...
            this.optionField.children(".option").click( function() {
                ...
                {
                    if (Widget.options.iconAsValue)
                        ValueField.empty().append("<img class=\"combobox_valueimg\" src=\"" + Icons[OptionValue] + "\"/>");
                    else
        ...
        editable: function( aEnable ) {
            ...
            else
            {
                ...
                {
                    if (this.options.iconAsValue)
                        ValueField.append("<img class=\"combobox_valueimg\" src=\"" + this.options.icons[SelectedElement.val()] + "\"/>");
                    else
