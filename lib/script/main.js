...
function onChangeConfig()
{
    ...
    if ( gSite.Logout )
    {
        var HTMLString = "<form id=\"logout\" method=\"post\" action=\"index.php\">";
        HTMLString    += "<input type=\"hidden\" name=\"nocheck\"/>";
        HTMLString    += "<input type=\"hidden\" name=\"logout\"/>";
        HTMLString    += "<button onclick=\"return onLogOut()\" class=\"button_logout\">"+L("Logout")+"</button>";
        HTMLString    += "</form>";
        
        $(".logout").append(HTMLString);
