...
function showTooltipRaidInfoById( a_RaidId )
{
...
function showTooltipRaidComment( a_RaidId )
{
    var raidElement = $("#raid"+ a_RaidId);
    var container = $("#info_text");
...
function commitAttend( a_Index, a_Raid, a_CalendarView, a_Comment )
{
    var Parameters = {
    ...
    asyncQuery( "raid_attend", Parameters, ( a_CalendarView ) ? generateCalendar : loadAllRaids );
}
...
function triggerAttend( selectObj, a_CalendarView )
{
    ...
    {
        prompt( L("WhyAbsent"), L("SetAbsent"), L("Cancel"), function(a_Text) {
            commitAttend( selectObj.value, selectObj.id.substr(6), a_CalendarView, a_Text );
        });
    }
    else
    {
        commitAttend( selectObj.value, selectObj.id.substr(6), a_CalendarView, "" );
    }
...
function triggerUpdateComment( a_ButtonElement, a_RaidId, a_CalendarView )
{
    ...
    asyncQuery( "comment_update", Parameters, ( a_CalendarView ) ? generateCalendar : loadAllRaids );
}
...
function showTooltipAttendee( a_SlotElement, a_Image, a_Name, a_Text, a_MakeSticky, a_OnHide )
{
    if ( !toggleStickyState(a_SlotElement, a_MakeSticky)  )
    {
        ...
        var filteredText = a_Text.replace(/(http:\/\/\S+)/g, "<a href=\"$1\" target=\"_blank\">$1</a>");
        
        var HTMLString = "<div class=\"playerTooltipIcon\" style=\"background-image: url('"+a_Image+"')\"></div>";
        HTMLString += "<div class=\"playerTooltipText\"><div class=\"name\">"+a_Name+"</div>"+filteredText+"</div>";
...
function showTooltipSlackers( a_SlotElement, a_Player, a_Undecided, a_MakeSticky )
{
    ...
    if ( !toggleStickyState(a_SlotElement, a_MakeSticky)  )
    {
        ...
        var filteredText = a_Player.comment.replace(/(http:\/\/\S+)/g, "<a href=\"$1\" target=\"_blank\">$1</a>");

        var iconClass = "playerTooltipIconLarge";
        if ( a_Player.characters.length > 1 )
          iconClass += " clickable";
        ...
        if ( a_Player.characters.length > 1 )
        {
            HTMLString += "<div class=\"twinkcount\">1/"+(a_Player.characters.length)+"</div>";
        }
        ...
        HTMLString += "<div style=\"float: left\">";
        HTMLString += "<div class=\"playerTooltipRoleIcon\" style=\"background-image: url('images/roles/"+g_RoleIdents[a_Player.firstRole]+".png')\"><div class=\"mainbadge\"></div></div>";
        HTMLString += "<div class=\"playerTooltipRoleIcon\" style=\"background-image: url('images/roles/"+g_RoleIdents[a_Player.secondRole]+".png')\"></div>";
        HTMLString += "</div>";
        ...
        if ( a_Player.characters.length > 1 )
        {
            ...
            $(".playerTooltipIconLarge").click( function() {
                ...
                if ( charIdx >= a_Player.characters.length )
                    charIdx = 0;

                $(this).data("index",charIdx);
                $(this).css("background-image", "url(images/classesbig/"+a_Player.characters[charIdx].className+".png)");
                $(this).siblings().children(".playerTooltipRoleIcon").first().css("background-image", "url(images/roles/"+g_RoleIdents[a_Player.characters[charIdx].firstRole]+".png)");
                $(this).siblings().children(".playerTooltipRoleIcon").last().css("background-image", "url(images/roles/"+g_RoleIdents[a_Player.characters[charIdx].secondRole]+".png)");

                HTMLString = "<div class=\"twinkcount\">"+(charIdx+1)+"/"+(a_Player.characters.length)+"</div>";
                
                if ( charIdx === 0 )
                    HTMLString += "<div class=\"mainbadge\"></div>";
                
                $(this).empty().append( HTMLString );                
                $(".playerTooltipText .name").empty().append( a_Player.characters[charIdx].name );
                
                if ( gUser.isAdmin && (a_SlotElement.data("setup_info") != null) )
                {
                    a_SlotElement.siblings(".playerName:first").empty().append( a_Player.characters[charIdx].name );
                    a_SlotElement.data("setup_info", charIdx);
                }                
            });
        }
...
function showTooltipClassList( a_ParentElement )
{
    ...
    else
    {
        ...
        $(".class_select").click( function() {
            ...
            var roleDiv = a_ParentElement.nextAll(".role_group");
            
            var charIdx = parseInt( a_ParentElement.parent().attr("id").substr(4), 10 );
...
function showTooltipRoleList( a_ParentElement, a_EditRole1 )
{
    ...
    else
    {
        ...
        var charIdx = parseInt( a_ParentElement.parent().parent().attr("id").substr(4), 10 );
        var charList = $("#charlist").data("characters");        
        var classString = charList.mCharacters[charIdx].charClass;

        if ( (a_EditRole1 && (charList.mCharacters[charIdx].role1 < 0)) ||
