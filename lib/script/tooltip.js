...
function showTooltipCharChooser( aSlotElement, aPlayer, aCurrentRole, aMakeSticky, aOnHide )
{
    ...
    if ( !toggleStickyState(aSlotElement.parent(), aMakeSticky)  )
    {
        ...
        for (var i=0; i<aPlayer.characters.length; ++i)
        {
            ...
            var CharClasses = Char.className.split(":");
        
            if ( gConfig.ClassMode == "multi" )
            {
                for (var c=0; c<CharClasses.length; ++c)
                {
                    var ClassIdx = gConfig.ClassIdx[CharClasses[c]];
            ...
            {
                var ClassIdx = gConfig.ClassIdx[CharClasses[0]];
                ...
                CharsByRole[Char.firstRole].push({ charIdx: i, classIdx: ClassIdx });
                ...
                if (Char.secondRole != Char.firstRole)
                {
                    ...
                    CharsByRole[Char.secondRole].push({ charIdx: i, classIdx: ClassIdx });
                }
            }
        }
        ...
        $(".switch_button", Container).click( function() {
            ...
            aPlayer.switchTo(SelectedChar.charIdx, SelectedChar.classIdx);
            hideTooltip(); 
        });
        ...
        var onUpdateCharField = function() 
        {
            ...
            {            
                var RoleInfo = L(gConfig.RoleNames[gConfig.RoleIdents[ActiveChar.firstRole]]) + 
                    ((ActiveChar.firstRole != ActiveChar.secondRole)
                        ? ", " + L(gConfig.RoleNames[gConfig.RoleIdents[ActiveChar.secondRole]])
                        : "");
            ...
            {
                var Classes = ActiveChar.className.split(":");
                DetailField.empty();
                ...
                for (var i=0; i<Classes.length; ++i)
                {
                    ...
                    {
                        var cidx = gConfig.ClassIdx[Classes[i]];
            ...
            if ((aPlayer.status == "unavailable") || (aPlayer.status == "undecided"))
            {
        ...
        {
            ElementOffset.left -= Tooltip.width() + aSlotElement.width() - 27;
