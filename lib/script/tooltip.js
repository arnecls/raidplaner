...
function showUserTooltip( a_UserElement, a_Sticky )
{
    ...
    else
    {
        ...
        HTMLString += "<div style=\"display: block; width: 160px; text-align: left; font-size: 11px; margin-top: 10px; margin-bottom: 5px;\" id=\"usr_reconnect\">"+L("LinkUser")+"</div>";
        
        container.empty().append( HTMLString );

        var userId = a_UserElement.attr("id");
        
        $("#grp_move").change( function() { moveUserToGroup( userId, $("#grp_move").value); hideTooltip(); } );
        $("#grp_move").combobox({ darkBackground: true });
        
        $("#usr_reconnect").click( function() { linkUser(userId); } );
        $("#usr_reconnect").button({ 
            icons: { secondary: "ui-icon-link" }
        });

        showNewTooltip();

        elementOffset.left += refElement.width();
        elementOffset.top  += (refElement.height() / 2) - (tooltip.height() / 2) + 20;
