...
{	
	...
	//$("#tooltip").data("sticky", a_MakeSticky );
...
function showAttendeeTooltip( a_SlotElement, a_MakeSticky )
{
	if ( g_User == null ) 
		return;
	...
	if ( !toggleStickyState(a_SlotElement, a_MakeSticky)  )
	{
		...
		if ( $.browser.msie && ($.browser.version < 8) )
			container.children().first().data( "src", a_SlotElement.children(".comment") );
