...
function showSlackersTooltip( a_SlotElement, a_Player, a_Undecided, a_MakeSticky )
{
	...
	if ( !toggleStickyState(a_SlotElement, a_MakeSticky)  )
	{
		...
		HTMLString += "<div class=\""+iconClass+"\" style=\"background-image: url('images/classesbig/"+a_Player.class+".png')\">";
		
		if ( a_Player.twinks.length > 0 )
		{
            HTMLString += "<span class=\"twinkcount shadow_default\">1/"+(a_Player.twinks.length+1)+"</span>";
		}
		
		HTMLString += "</div>";
		HTMLString += "<div class=\"playerTooltipIcon\" style=\"background-image: url('images/roles/"+g_RoleIdents[a_Player.firstRole]+".png')\"></div>";
 		HTMLString += "<div class=\"playerTooltipIcon\" style=\"background-image: url('images/roles/"+g_RoleIdents[a_Player.secondRole]+".png')\"></div>";
 		HTMLString += "</div>";
		...
		{		
    		...
    		$(".playerTooltipIconLarge").click( function() {
                ...
                $(this).css("background-image", "url(images/classesbig/"+characters[charIdx].class+".png)");
                $(this).siblings().first().css("background-image", "url(images/roles/"+g_RoleIdents[characters[charIdx].firstRole]+".png)");
                $(this).siblings().last().css("background-image", "url(images/roles/"+g_RoleIdents[characters[charIdx].secondRole]+".png)");
               
                HTMLString = "<span class=\"twinkcount shadow_default\">"+(charIdx+1)+"/"+(a_Player.twinks.length+1)+"</span>";
                $(this).empty().append( HTMLString );
                
                if ( charIdx > 0 )
                    HTMLString = "<div class=\"twinkbadge\"></div>"+characters[charIdx].name;
                else
                    HTMLString = characters[charIdx].name;
                    
                $(".playerTooltipText .name").empty().append( HTMLString );
    		});
    	}
...
function showTooltipClassList( a_ParentElement )
{
	...
	{	
		...
		$(".class_select").click( function() { 
			...
			var defaultRoleId = g_RoleIds[ g_Classes[g_ClassIdx[classIdent]].roles[0] ];
			...
			a_ParentElement.parent().data("charData").role1 = defaultRoleId;
			a_ParentElement.parent().data("charData").role2 = defaultRoleId;
			
			a_ParentElement.parent().children("span").children(".role").attr("src", "images/roles/"+defaultRoleIdent+".png" );
...
{	
	...
	else
	{
		...
		$(".class_select").click( function() { 
			...
			a_ParentElement.attr("src", "images/roles/" + roleIdent + ".png" );
...
function showUserTooltip( a_UserElement, a_Sticky )
{
	...
	{	
		...
		elementOffset.top  += (refElement.height() / 2) - (tooltip.height() / 2) + 8;
