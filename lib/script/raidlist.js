...
function generateRaidList( aXHR )
{
    ...
    $.each(aXHR.raid, function(index, value) {
        ...
        var StatusOk =  (value.status == "ok" ? '<span class="ui-icon ui-icon-check"></span>' : '');
        ...
        var RoleClass = (role.length > 0 ? ' ' + role : '');
        ...
        HTMLString += "<div class=\"overlayStatus " + OverlayClass + RoleClass + "\">";
        HTMLString += StatusOk + "</div>";
        HTMLString += "</span>";

        HTMLString += "<span class=\"raidInfo\">";
        HTMLString += "<div class=\"location\">" + value.location + ((value.mode != "all") ? "(" + value.size + ")" : "") + "</div>";
        ...
        if (value.mode != "all") {
            ...
            HTMLString += "<div style=\"line-height: 2.5em\">" + value.attended + " " + L("Attendants") + "</div>";
        }
