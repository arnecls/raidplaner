...
function generateRaidList( aXHR )
{
    ...
    $.each(aXHR.raid, function(index, value) {
        ...
        for ( var i=0; Complete && (i < gConfig.RoleIds.length); i++ )
        {
        ...
        {
            for ( i=0; i < gConfig.RoleIds.length; i++ )
            {
                var Required   = parseInt( value["role"+i+"Slots"], 10 );
                var SlotsInUse = parseInt( value["role"+i], 10 );
                ...
                {
                    HTMLString += "<div class=\"setupInfoSlot\" style=\"background-image: url("+gConfig.RoleImages[i]+")\">";
