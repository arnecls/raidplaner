...
function displaySettingsAbout( a_Message )
{
	...
	var patchLevel = String.fromCharCode( "a".charCodeAt(0) + parseInt((g_SiteVersion - parseInt(g_SiteVersion))*10) );
	
	HTMLString += "<div class=\"version\">";
	HTMLString += "Version " + parseInt(g_SiteVersion / 100) + "." + parseInt((g_SiteVersion % 100) / 10) + "." + parseInt(g_SiteVersion % 10) + patchLevel + "<br/>";
...
{	
	if ( g_SiteVersion < a_JSONData.version )
	{
		...
		$("#update_check").before( "<a href=\"http://code.google.com/p/ppx-raidplaner/downloads/list\" style=\"font-size: 12px\">" + L("VisitProjectPage") + "</a><br/>" );
	...
	else
	{
		...
		$("#update_check").before( "<span style=\"font-size: 12px; color: #666\">" + L("UpToDate") + "</span><br/>" );
