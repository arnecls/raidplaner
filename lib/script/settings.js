...
function generateSettingsRaid( aXHR )
{
    ...
    HTMLString += "<div class=\"settingField\"><select id=\"raidsize\" style=\"width: " + ((gSite.TimeFormat == 24) ? 48 : 64) + "px\">";

    $.each(gGame.Groups, function(aSize, aGroupInfo)
    {
        HTMLString += "<option value=\""+aSize+"\"" + ((Settings["RaidSize"].number == aSize) ? " selected" : "" ) + ">"+aSize+"</option>";
    });
    ...
    $.each(aXHR.game, function(index, value) {
        ...
        HTMLString += "<option theme=\""+Theme+"\" value=\"" + File + "\"" + ((Settings["GameConfig"].text == File) ? " selected" : "" ) + ">" + Name + "</option>";
    });
    ...
    var switchGame = function(aGameTheme) {
        ...
        $.each(aXHR.theme, function(index, value) {
            if (value.games.indexOf(aGameTheme) > -1)
            {
                var Name = value.name;
                var File = value.file;
    ...
    $("#game").change( function() {
        var GameTheme = $("option:selected",this).attr("theme");
        switchGame(GameTheme); 
    });
    
    switchGame(Settings["GameConfig"].text);
...
{
    var HTMLString = "<div id=\"statistics\" class=\"content_area\">";
