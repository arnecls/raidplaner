function CGroup( aName )
{
    ...
    this.addUser = function( aId, aLogin, aBindingActive, aBinding )
    {
        this.mUsers.push( {
            id             : aId,
            login          : aLogin,
...
function onLinkUserReturn( aXMLData )
{
    var Message = $(aXMLData).children("messagehub:first");
    var UserId  = Message.children("userid:first").text();
    var Group   = Message.children("group:first").text();
    var Binding = Message.children("binding:first").text();
...
function generateSettingsUsers( aMessage )
{
    ...
    aMessage.children("user").each( function() {
        var GroupName = $(this).children("group:first").text();
    
        if ( GroupName == "none" )
        {
            Banned.addUser( $(this).children("id:first").text(),
                $(this).children("login:first").text(),
                $(this).children("bindingActive:first").text() == "true",
                $(this).children("binding:first").text() );
        }
        else if ( GroupName == "member" )
        {
            Member.addUser( $(this).children("id:first").text(),
                $(this).children("login:first").text(),
                $(this).children("bindingActive:first").text() == "true",
                $(this).children("binding:first").text() );
        }
        else if ( GroupName == "raidlead" )
        {
            Raidlead.addUser( $(this).children("id:first").text(),
                $(this).children("login:first").text(),
                $(this).children("bindingActive:first").text() == "true",
                $(this).children("binding:first").text() );
        }
        ...
        {
            Admin.addUser( $(this).children("id:first").text(),
                $(this).children("login:first").text(),
                $(this).children("bindingActive:first").text() == "true",
                $(this).children("binding:first").text() );
        }
...
function generateSettingsLocation( aMessage )
{
    ...
    aMessage.children("locationimage").each( function(aIndex) {
        ...
        HTMLString += "<img class=\"clickable\" src=\"images/raidsmall/" + $(this).text() + "\" onclick=\"applyLocationImageExternal(this, true)\" style=\"width: 32px; height: 32px; margin-right: 5px;\"/>";
        ++NumImages;
    });
    ...
    aMessage.children("location").each( function() {

        var LocationId = parseInt( $(this).children("id:first").text(), 10 );
        var LocationImage = $(this).children("image:first").text();
        ...
        HTMLString += "<input class=\"locationname\" type=\"text\" value=\"" + $(this).children("name:first").text() + "\"/>";
...
function generateSettingsRaid( aMessage )
{
    var PurgeRaidTime   = 0;
    ...
    aMessage.children("setting").each( function() {
    
        var SettingName = $(this).children("name:first").text();

        if ( SettingName == "PurgeRaids" )
        {
            var ShortTime = calculateShortTime( parseInt( $(this).children("intValue:first").text(), 10 ) );
        ...
        Settings[ SettingName ] = {
            text   : $(this).children("textValue:first").text(),
            number : parseInt( $(this).children("intValue:first").text(), 10 )
        };
    });
    ...
    aMessage.children("theme").each( function() {
...
function generateSettingsStats( aMessage )
{
    ...
    var NumRaids = parseInt( aMessage.children("numRaids").text(), 10 );
    var BarSize  = 740;

    HTMLString += "<div style=\"clear: left; padding-top: 15px\">";

    aMessage.children("attendance").each( function() {

        HTMLString += "<div style=\"clear: left\">";
        HTMLString += "<div class=\"name\">" + $(this).children("name:first").text() + "</div>";
        HTMLString += "<div class=\"bar\"><span class=\"start_bar\"></span>";

        var NumOk      = parseInt( $(this).children("ok:first").text(), 10 );
        var NumAvail   = parseInt( $(this).children("available:first").text(), 10 );
        var NumUnavail = parseInt( $(this).children("unavailable:first").text(), 10 );
...
function generateSettingsAbout( aMessage )
{
    var HTMLString = "<div id=\"about\">";
    var PatchLevel = parseInt(Math.ceil((gSiteVersion - parseInt(gSiteVersion, 10))*10), 10);
...
function generateSettings( aXMLData )
{
    var Message = $(aXMLData).children("messagehub:first");
...
function onUpdateCheckReturn( aJSONData )
{
    ...
    if ( gSiteVersion < aJSONData.version )
    {
        ...
        notify( L("NewVersionAvailable") + "<br/><span style=\"font-size: 26px\">" + aJSONData.major + "." + aJSONData.minor + "." + aJSONData.patch + "</span>" );
    }
    else
    {
