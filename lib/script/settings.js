...
function generateSettingsRaid( aXHR )
{
    ...
    $.each(aXHR.game, function(index, value) {
        var Name = value.name;
        var File = value.file;
        HTMLString += "<option value=\"" + File + "\"" + ((Settings["GameConfig"].text == File) ? " selected" : "" ) + ">" + Name + "</option>";
    });
    ...
    var switchGame = function(aGame) {
        ...
        $.each(aXHR.theme, function(index, value) {
            if (value.games.indexOf(aGame) > -1)
            {
                var Name = value.name;
                var File = value.file;
    ...
    $("#game").change( function() {
        var file = $("option:selected",this).val();
        
        $.each(aXHR.game, function(aIndex, aGame){
            if (aGame.file == file)
                switchGame(aGame.id);  
        });  
    });
