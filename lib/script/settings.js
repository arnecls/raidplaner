function CGroup( a_Name )
{
    ...
    this.updateUnlinkedArray = function(a_Array)
    {
        for ( var i=0; i<this.users.length; ++i)
        {
            if ( (this.users[i].binding != "none") && !this.users[i].bindingActive )
                a_Array.push( this.users[i].id );
        }
    }
    ...
    this.refreshList = function()
    {
        ...
        for (i=0; i<this.users.length; ++i)
        {
            ...
            var bindingClass  = (this.users[i].bindingActive) ? "binding_active" : "binding";
            var isReserved    = (this.users[i].id == 1);
            var isLocked      = isReserved || (this.users[i].bindingActive && (bindingString != "none"));
        ...
        if ( HTMLString == "" )
        {
            // Fix for chrome display bug
            HTMLString += "<div style=\"height:380px\"></div>";
        }
        
        var listElement = $("#" + this.name);
...
function onLinkUserReturn( a_XMLData )
{
    ...
    if ( sourceHelper != null )
    {
        ...
        var userIdx = sourceHelper.getUserIdxById( parseInt(userId) );
        var user = sourceHelper.users[userIdx];
...
function displaySettingsAbout( a_Message )
{
    ...
    var patchLevel = parseInt(Math.ceil((g_SiteVersion - parseInt(g_SiteVersion))*10));
    var patchLevelChar = (patchLevel == 0) ? "" : String.fromCharCode( "a".charCodeAt(0) + (patchLevel-1) );

    HTMLString += "<div class=\"version\">";
    HTMLString += "Version " + parseInt(g_SiteVersion / 100) + "." + parseInt((g_SiteVersion % 100) / 10) + "." + parseInt(g_SiteVersion % 10) + patchLevelChar + "<br/>";
...
function triggerSettingsUpdate()
{
    ...
    var unlinkedArray = new Array();
    banned.updateUnlinkedArray(unlinkedArray);
    member.updateUnlinkedArray(unlinkedArray);
    raidlead.updateUnlinkedArray(unlinkedArray);
    admin.updateUnlinkedArray(unlinkedArray);    
