function CGroup( a_Name )
{
    ...
    this.updateLinkageArrays = function(a_UnlinkedArray, a_RelinkedArray)
    {
        for ( var i=0; i<this.users.length; ++i)
        {
            if ( this.users[i].changedLinkage )
            {
                if (this.users[i].bindingActive)
                    a_RelinkedArray.push( this.users[i].id );
                else
                    a_UnlinkedArray.push( this.users[i].id );
            }
        }
    }
