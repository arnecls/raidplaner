...
function CRaidMemberList()
{
    ...
    this.addPlayer = function( aAttendee )
    {
        ...
        if (aAttendee.classname[0] == "random")
        {
    ...
    this.generateEmptySlot = function( aClipStatus )
    {
        ...
        HTMLString += "<div class=\"emptySlot "+LayoutClass+"\" style=\"background-image: url(lib/layout/images/"+gConfig.Roles[aClipStatus.roleId].style+".png)\">";
        HTMLString += "</div>";
    ...
    this.generateAddRandomSlot = function( aClipStatus, aOverEmptySlot )
    {
        ...
        if (aOverEmptySlot)
        {
            var RoleImage = gConfig.Roles[aClipStatus.roleId].style+"_rnd.png";
    ...
    this.generateRoleList = function( aRoleId, aColumns, aRequiredSlots )
    {
        ...
        var Role = gConfig.Roles[aRoleId];
        var HTMLString = "";
        ...
        if (aRoleId == gConfig.Roles.length-1)        
            HTMLString += "<div class=\"roleList\" style=\"margin-right:0\">";
        else
            HTMLString += "<div class=\"roleList\">";
        
        HTMLString += "<h3>"+Role.text+" <span class=\"attendance_count\">"+AttendedForRole+"/"+aRequiredSlots+"</span></h3>";
...
function validateRoleCounts()
{
    ...
    for ( var i=0; i < gConfig.Roles.length; ++i )
    {
        ...
        if ( (TotalMembers + RoleSlots > MaxAllowed) ||
             ((i == gConfig.Roles.length-1) && (TotalMembers + RoleSlots < MaxAllowed)) )
        {
...
function generateRaidSetup( aXHR )
{
    ...
    for ( var i=0; i<gConfig.Roles.length; ++i )
    {
        var NumCols  = gConfig.Roles[i].columns;
...
function onClickAbsentSlot(aEvent, aElement, aPlayer, aUndecided)
{
    ...
    if ((CharData !== null) && (CharData !== undefined))
    {
        ...
        aPlayer.activeRole = CharData.roleIdx;
    ...
    else
    {
        ...
        var mainRole = (gConfig.ClassMode == "multi") 
            ? gConfig.RoleIdx[gConfig.Classes[gConfig.ClassIdx[aPlayer.className[0]]].defaultRole]
            : aPlayer.firstRole;
...
function generateRaidSettings( aXHR )
{
    ...
    $.each(aXHR.slots, function(index, value) {
        if ( index < gConfig.Roles.length )
        {
            HTMLString += "<div class=\""+((index == 0) ? "settingLine settingNewSection" : "settingLine")+"\"><div class=\"settingLabel\">"+L("RequiredForRole")+" \""+gConfig.Roles[index].text+"\"</div>";
            HTMLString += "<div class=\"settingField\"><input id=\"slotCount"+index+"\" style=\"width:24px;\" value=\""+value+"\" type=\"text\"/></div></div>";
        }
    });
