...
function generateRaid( aXHR )
{
    ...
    if ( gUser.isRaidlead )
    {
        HTMLString += "<button id=\"exportButton\">" + L("ExportXML") + "</button>";
    ...
    if ( gUser.isRaidlead)
    {
        ...
        $("#exportButton").button({ icons: { secondary: "ui-icon-extlink" }})
            .css("right",$("#applyButton").width()+50)
            .click( function() {
                var LocationName = aXHR.locationname.replace(/\s/g,"_");
                window.location = "lib/apihub.php?query=raid&raid="+RaidId+"&attends=true&token="+aXHR.token+"&format=xml&as=raid_"+RaidId+".xml";
            });
