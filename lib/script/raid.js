...
function CRaidMemberList()
{
    ...
    {
        // Returns an array with the following elements. Elements >= 3 depend
        // on the flags value.
        //
        // [+0] PlayerId
        // [+1] Status
    ...
    this.ChangePlayerName = function( a_PlayerId, a_Name )
    {
        ...
        if (a_Name != this.players[pIdx].name)
        {
            this.players[pIdx].name = a_Name;            
            this.players[pIdx].flags |= PlayerFlagModified | PlayerFlagName;
    ...
    this.DisplayAddRandomSlot = function( a_ClipStatus, a_OverEmptySlot )
    {
        var HTMLString = "";
        var layoutClass = ((a_ClipStatus.clipItemCount % a_ClipStatus.colsPerClip) == 0) ? "break" : "nobreak";
    ...
    this.UpdateClipStatus = function( a_ClipStatus )
    {
        ...
        if ( a_ClipStatus.itemCount < a_ClipStatus.displayCount )
        {
            ...
            {
                HTMLString += this.DisplayNextClipButton(a_ClipStatus);
            }
        }
    ...
    this.DisplayNextClipButton = function( a_ClipStatus )
    {
        var HTMLString = "";
        var layoutClass = (a_ClipStatus.colsPerClip == 1) ? "break" : "nobreak";

        HTMLString += "<div class=\"clipchange clickable "+layoutClass+"\" onclick=\"raidShowClip('role"+a_ClipStatus.roleId+"clip"+(a_ClipStatus.currentId+1)+"')\"></div>";
        HTMLString += "</div>";
        HTMLString += "<div class=\"clip\" id=\"role"+a_ClipStatus.roleId+"clip"+(a_ClipStatus.currentId+1)+"\">";
        HTMLString += "<div class=\"clipchange clickable break\" onclick=\"raidShowClip('role"+a_ClipStatus.roleId+"clip"+a_ClipStatus.currentId+"')\"></div>";
    ...
    this.DisplayRole = function( a_RoleId, a_Columns, a_RequiredSlots )
    {
        ...
        var adjust = (rowIdx - parseInt(rowIdx) == 0) ? 0 : 1;
        var newRow = parseInt(rowIdx)+adjust;
        
        if ( newRow == clipStatus.rowsPerClip )
        {
            HTMLString += this.DisplayNextClipButton(clipStatus);
        }
        else
        {        
        ...
        if ( g_User.isRaidlead )
        {
            ...
            HTMLString += this.DisplayAddRandomSlot( clipStatus, overEmptySlot );
        }
            ...
            drop: function() {                
                if ( player.charId > 0 )
                {
                    prompt(L("AbsentMessage"), L("MarkAsAbesent"), L("Cancel"), function(a_Comment) {
                        a_Source.draggable("option", "revert", false);
                        a_Source.draggable("destroy").detach();
                        playerList.AbsentPlayer(a_PlayerId, "["+g_User.name+"] "+a_Comment);                
                    });
                }
                else
                {
    ...
    this.UpgradePlayer = function( a_PlayerId )
    {
        ...
        this.players[pIdx].flags |= PlayerFlagModified;
    ...
    this.AbsentPlayer = function( a_PlayerId, a_Comment )
    {
        ...
        if ( player.charId > 0 )
        {
            ...
            $("#absentList").replaceWith(this.DisplayAbsent(3));
...
function displayRaidSetup( a_RaidXML )
{
    ...
    HTMLString = "<div id=\"absent_drop\">"+L("MakeAbsent")+"</div>";
    $("#raidsetup").append(HTMLString);
...
function onClickAbsentSlot(a_Event, a_Element, a_Player, a_Undecided)
{
    ...
    if (charIdx != null)
    {
        ...
        a_Player.comment = L("SetupBy") + g_User.name;
        ...
        if ( a_Undecided )
        {
            $("#undecidedList").replaceWith(playerList.DisplayUndecided(3));
            onUpdateUndecidedList(playerList);
        }
        else
        {
            $("#absentList").replaceWith(playerList.DisplayAbsent(3));
    ...
    {        
        ...
        showSlackersTooltip( a_Element, a_Player, false, true );
...
function onUpdateAbsentList(a_PlayerList)
{
    ...
    $("#absentList").children(".clip").children(".benchSlotComment, .benchSlot").each( function() {
        ...
        var player  = a_PlayerList.players[pIdx];
