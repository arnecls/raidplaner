...
function displayRaidSettings( a_MessageXML, a_RaidXML )
{
    ...
    raidSlots.each( function(index) {
        if ( index < g_RoleIds.length )
        {
            ...
            HTMLString += "<div class=\"settingField\"><input type=\"text\" id=\"slotCount"+index+"\" class=\"settingEdit\" style=\"width:24px;\" value=\""+$(this).text()+"\"/></div>";
        }
    });
...
function triggerRaidUpdate()
{
    ...
    var Parameters = {
        ...
        month        : $("#raidsettings").data("info").month,
        day          : $("#raidsettings").data("info").day,
        year         : $("#raidsettings").data("info").year,
        stage        : $("#raidstatus").val(),
        mode         : $("#raidmode").val(),
	...
	AsyncQuery( "raid_update", Parameters, function() {
	   changeContext(hash);
	}); 
}