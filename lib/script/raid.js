...
function onRaidDataChange()
{
    onUIDataChange();
    $("#exportButton").button("option", "disabled", true);
}
...
function generateRaidSettings( aXHR )
{
    ...
    HTMLString += "</select>";
    HTMLString += "<button id=\"exportFile\">" + L("ExportFile") + "</button>";
    HTMLString += "<button id=\"exportClipboard\">" + L("ExportClipboard") + "</button>";
    HTMLString += "</div></div>";
    ...
    $("#exportClipboard").button({ icons: { secondary: "ui-icon-clipboard" }});
    ...
    $("#raidoptions select").change( onRaidDataChange );
    $("#raidoptions textarea").change( onRaidDataChange );
