...
function CRaidMemberList()
{
        ...
        var NewPlayer = {
            ...
            setAbsent   : function() {
                            ...
                            prompt(L("AbsentMessage"), L("MarkAsAbesent"), L("Cancel"), function(aComment) {
                                ...
                                self.absentPlayer(PlayerId, "</br>["+gUser.name+"] " + aComment);
                            });
                          }
        };
    ...
    this.generateRoleList = function( aRoleId, aColumns, aRequiredSlots )
    {
        ...
        if (SetupForRole >= aRequiredSlots)
            HTMLString += "<h3 class=\"glow_complete\">";
        else
            HTMLString += "<h3>";
        
        HTMLString += L(Role.name)+" <span class=\"attendance_count\">"+SetupForRole+"/"+aRequiredSlots+"</span></h3>";
    ...
    this.bindClips = function( aParent )
    {
        ...
        if ( gUser.isRaidlead )
        {
            ...
            Clips.children(".benchSlot, .benchSlotComment").each( function() {
                var PlayerId  = parseInt( $(this).attr("id").substr(2), 10 );
                var PlayerIdx = PlayerList.getPlayerIndex(PlayerId);
                var Player    = PlayerList.mPlayers[PlayerIdx];
                var Name      = Player.name;
    ...
    this.updateRoleList = function( aRoleId )
    {
        ...
        var ClipIdIdx = RoleConf.clip.lastIndexOf("_clip")+5;
        var ClipId = parseInt( RoleConf.clip.substr(ClipIdIdx), 10 );
        var NumClips = RoleList.children(".clip").length;

        if ( ClipId >= NumClips )
        {
            RoleConf.clip = RoleConf.clip.substr(0,ClipIdIdx) + (NumClips-1);
        }
    ...
    this.switchPlayer = function( aPlayerId, aCharIdx, aRoleId, aClassId )
    {
        ...
        if (aCharIdx !== null)
        {
                ...
                (Player.activeRole != aRoleId))
            {
                ...
                Player.flags |= cPlayerFlagCharId | cPlayerFlagModified | cPlayerFlagComment;
            }
        }
    ...
    this.absentPlayer = function( aPlayerId, aComment )
    {
        ...
        if ( Player.charId > 0 )
        {
            ...
            Player.activeClass = Character.className[0]; 
...
function generateRaidInfo( aXHR, aAppendTo )
{
    ...
    var playerComments = '';
    var playerCancelledComments = '';
    var CommentCounts = {};
    CommentCounts.Unavailable = 0;
    CommentCounts.Available = 0;

    $.each(aXHR.slots, function(aRole, aSize) {
       RaidSize += aSize;
    });
    
    $.each(aXHR.attendee, function(idx, val) {
      if (val['comment'] != '') {
        if (val['status'] == 'unavailable') {
          playerCancelledComments += '<span style="font-weight: bold">' + val.name + ': </span>' + val.comment + '<br />';
          CommentCounts.Unavailable++;
        }
        else
        {
          playerComments += '<span style="font-weight: bold">' + val.name + ': </span>' + val.comment + '<br />';
          CommentCounts.Available++;
        };
      };
    });
    ...
    var HTMLString = $("<div class=\"raidinfo\">" +
      "<img src=\"themes/icons/"+gSite.Iconset+"/raidbig/" + RaidImage + "\" class=\"raidicon\">" +
      "<div class=\"raidname\" style=\"font-size: "+NameFontSize+"px\">" + RaidName + "</div>" +
      "<div class=\"datetime\">" + formatDateTimeStringUTC(StartDate,StartTime) + " - " + formatTimeStringUTC(EndDate,EndTime) +
      " " + formatDateOffsetUTC(StartDate, StartTime) + "</div>" +
      "<div class=\"raidsize\"><span class=\"attends\">" + Attended + "</span> / " + RaidSize + " " + L("Players") + "</div>");
    
    //if (RaidComment != "")
        HTMLString.append('<div class="raidcomment"><div id="tabComments"><ul>' +
        '<li><a href="#raidComment">Raid</a></li>' +
        '<li><a href="#playerComments">Player [' + CommentCounts.Available + ']</a></li>' +
        '<li><a href="#playerCancelledComments">Cancelled [' + CommentCounts.Unavailable + ']</a></li></ul>' +
        '<div id="raidComment">' + RaidComment + '</div>' +
        '<div id="playerComments">' + playerComments + '</div>' +
        '<div id="playerCancelledComments">' + playerCancelledComments + '</div>' +
        '</div></div>');

    HTMLString.append("</div>");
    
    HTMLString.find('#tabComments').tabs();
...
function updateRaidBadges(aPlayerList)
{
    var absentCount = 0;
    var attendedCount = 0;
    var setupCount = 0;
    ...
    $("#attenders_badge").empty();

    if (absentCount > 0)
        $("#slackers_badge").append("<span style=\"color: #f00\">" + absentCount + "</span>");
        
    if (setupCount > 0)
        $("#attenders_badge").append("<span style=\"color: #0f0\">" + setupCount + "</span>");
        
    $(".raidsize .attends").empty().append(attendedCount);
}
...
function generateRaidSettings( aXHR )
{
    ...
    HTMLString += "<div class=\"settingLine settingNewSection\"><div class=\"settingLabel\">"+L("Export")+"</div>";
    HTMLString += "<div class=\"settingField\"><select id=\"exportFormat\" style=\"width: 60px\">";
    HTMLString += "<option value=\"xml\">XML</option>";
    HTMLString += "<option value=\"json\" selected>JSON</option>";
    HTMLString += "</select>";
    HTMLString += "<button id=\"exportFile\" style=\"margin-left: 5px\">" + L("ExportFile") + "</button>";
    HTMLString += "<button id=\"exportClipboard\" style=\"margin-left: 5px\">" + L("ExportClipboard") + "</button>";
    HTMLString += "</div></div>";
    ...
    $("#exportFile").button({ icons: { secondary: "ui-icon-extlink" }})
        .click( function() {
            var Format = $("#exportFormat option:selected").val();
            var FileExt = (Format == "xml") ? ".xml" : ".json";
            window.location = "lib/apihub.php?query=raid&raid="+aXHR.raidId+"&attends=true&token="+aXHR.token+"&format="+Format+"&as=raid_"+aXHR.raidId+FileExt;
        });

    $("#exportClipboard").button({ icons: { secondary: "ui-icon-clipboard" }, disabled: true });
    
    var Clipboard = new ZeroClipboard( $("#exportClipboard"), {
        moviePath   : "lib/script/ZeroClipboard.swf",
        hoverClass  : "ui-state-hover",
        activeClass : "ui-state-active"
    });
    
    Clipboard.on("load", function() {
        $("#exportClipboard").button("option", "disabled", false);    
    });
    
    Clipboard.on("dataRequested", function(aSelf, aArgs) {
        var Format = $("#exportFormat option:selected").val();
        var Result = "";
        var Parameter = {
            query   : "raid",
        ...
        $.ajax({
            ...
            success  : function(aData, aStatus, aXHR) { Result = aXHR.responseText; },
            error    : onAsyncError
        });
        
        aSelf.setText(Result); 
    });
    
    Clipboard.on("complete", function(aSelf, aArgs) { if (aArgs.text != "") notify(L("CopyOk")); });
...
function generateRaid( aXHR )
{
    ...
    HTMLString += "<div id=\"setuptoggle\" class=\"tab_icon icon_setup clickable\"><div id=\"attenders_badge\" class=\"iconbadge\"></div></div>";
    HTMLString += "<div id=\"slackerstoggle\" class=\"tab_icon icon_slackers_off clickable\"><div id=\"slackers_badge\" class=\"iconbadge\"></div></div>";
