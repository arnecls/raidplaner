...
function CRaidMemberList()
{
    ...
    {
        var CharClasses = aPlayer.className.split(":");
        var ClassIdent = CharClasses[0];
    ...
    this.bindClips = function( aParent )
    {
        ...
        if ( gUser.isRaidlead )
        {
            ...
            Clips.children(".waitSlot, .waitSlotComment").each( function() {
                ...
                var Image     = "themes/icons/"+gSite.Iconset+"/classessmall/" + Player.classIdent() + ".png";
                var Slot      = $(this);
                ...
                var onClickFunction = function(aEvent) {
                    if ( PlayerList.mMode != "all" )
                    {
                        ...
                        Slot.unbind("click").click( function() {
                            var CharData = Icon.data("setup_info");
                            PlayerList.upgradePlayer(PlayerId, CharData.charIdx, CharData.classIdx);
            ...
            Clips.children(".activeSlot, .spilledSlot, .activeSlotComment, .spilledSlotComment").each( function() {
                ...
                var onClickFunction = function(aEvent) {
                    if ( PlayerList.mMode != "all" )
                    {
                        ...
                        Slot.unbind("click").click( function() {
                            var CharData = Icon.data("setup_info");
                            PlayerList.downgradePlayer(PlayerId, CharData.charIdx, CharData.classIdx);
    ...
    this.upgradePlayer = function( aPlayerId, aCharIdx, aClassIdx )
    {
        var PlayerIdx = this.getPlayerIndex( aPlayerId );
        var Player = this.mPlayers[PlayerIdx];
        ...
        if ((aCharIdx != null) && (Player.characters.length > 1))
        {
            ...
            if ((Player.charId != assignedChar.id) || (Player.activeClass != aClassIdx))
            {
    ...
    this.downgradePlayer = function( aPlayerId, aCharIdx, aClassIdx )
    {
        var PlayerIdx = this.getPlayerIndex( aPlayerId );
        var Player = this.mPlayers[PlayerIdx];
        var oldRole = Player.activeRole;
...
function onClickAbsentSlot(aEvent, aElement, aPlayer, aUndecided)
{
    ...
    else
    {
        var onAbortFunction = function() {
            ...
            aElement.css("background-image", "url(themes/icons/"+gSite.Iconset+"/classessmall/"+aPlayer.classIdent()+".png)");
