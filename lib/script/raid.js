...
function validateRoleCounts()
{
    ...
    for ( var i=0; i < gConfig.RoleIds.length; ++i )
    {
        var SlotElement = $("#slotCount"+i);
        var RoleSlots   = parseInt( SlotElement.val(), 10 );
        var MaxAllowed  = MaxMembers - (gConfig.RoleIds.length - (i+1));

        if ( (TotalMembers + RoleSlots > MaxAllowed) ||
             ((i == gConfig.RoleIds.length-1) && (TotalMembers + RoleSlots < MaxAllowed)) )
        {
...
function generateRaidInfo( aXHR, aAppendTo )
{
    ...
    var RaidSize    = 0;
    var NumRoles    = Math.min( gConfig.RoleNames.length, 5 );
...
function generateRaidSettings( aXHR )
{
    ...
    $.each(aXHR.slots, function(index, value) {
        if ( index < gConfig.RoleIds.length )
        {
