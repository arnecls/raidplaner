...
function CSlot( a_Name, a_Slots )
{
	...
	this.displayAsBench = function() 
	{
		...
		for ( i=0; i<this.players.length; ++i )
		{
			...
			if ( this.players[i].comment != "" )
				HTMLString += "<div class=\"badge\">";
			else
				HTMLString += "<div class=\"nobadge\">";
		...
		for ( i=0; i<this.unavail.length; ++i )
		{
			...
			HTMLString += "<span class=\"name unavailable shadow_default\">" + charName + "</span>";
		...
		for ( i=0; i<this.undecided.length; ++i )
		{
			...
			var charName = this.undecided[i].name;
			
			HTMLString += "<img class=\"class undecided\" src=\"images/classessmall/" + this.undecided[i].charClass + ".png\"/>";
			HTMLString += "<span class=\"name undecided shadow_default\">" + charName + "</span>";
			HTMLString += "</span>";
		}
		...
		if ( g_User.isRaidlead )
		{
			benchField.children(".slot")
				.dblclick( function( event ) { addToSlot($(this)); } )
		}
						
		benchField.children(".slot")
			.mouseover( function( event ) { showAttendeeTooltip( $(this), false ); } )
			
		benchField.children(".slot").children(".class")
			.click( function( event ) { showAttendeeTooltip( $(this).parent(), true ); event.stopPropagation(); } );
