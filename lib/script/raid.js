...
function addToBenchFromComment( a_Node )
{
	var sourceElementId = $(a_Node).attr("id");
	
	sourceElementId = sourceElementId.substring( 9, sourceElementId.length );
...
function moveSlotFromComment( a_Node )
{
	var sourceElementId = $(a_Node).attr("id");
	
	sourceElementId = sourceElementId.substring( 3, sourceElementId.length );
	var targetName  = sourceElementId.substr( 0, sourceElementId.indexOf("_") );
	
	sourceElementId = sourceElementId.substring( sourceElementId.indexOf("_")+1, sourceElementId.length );	
	var sourceName  = sourceElementId.substr( 0, sourceElementId.indexOf("_") );
	
	sourceElementId = sourceElementId.substring( sourceElementId.indexOf("_")+1, sourceElementId.length );	
	var sourceIndex = parseInt( sourceElementId );
	
	var playerObj = getSlotObj( sourceName ).removePlayer( sourceIndex );

	if ( playerObj.id != 0 )
	{
		getSlotObj( targetName ).addPlayerObj( playerObj );
	}
...
function addToSlotFromComment( a_Node )
{
	var sourceElementId = $(a_Node).attr("id");
...
{
	a_Node.droppable({ disabled: true })
...
function onDragStop()
{
	$("#slot_tank").removeClass("notarget");
	$("#slot_heal").removeClass("notarget");
	$("#slot_dmg").removeClass("notarget");
	
	$("#benchcontainer").removeClass("ui-state-disabled");
}
...
{
	clearDroppable( $("#slot_tank") );
	clearDroppable( $("#slot_heal") );
	clearDroppable( $("#slot_dmg") );
	
	$("#benchcontainer").droppable({ disabled: true });
}
...
function onDropFromBench( a_Event, a_Context )
{
	...
	var sourceName  = sourceElementId.substr( 0, sourceElementId.indexOf("_"));
	var sourceIndex = parseInt( sourceElementId.substr( sourceElementId.indexOf("_")+1 ) );	
	var slotName    = slotId.substring( slotId.indexOf("_")+1, slotId.length );
...
function CSlot( a_Name, a_Slots )
{
	...
	this.displayAsSlot = function() 
	{
		...
		for ( i=0; i<this.slots; ++i )
		{
			...
			if ( i < this.players.length )
			{
				...
				if ( !this.players[i].mainChar )
					HTMLString += "<div class=\"twinkicon\"></div>";
				...
				if ( g_User.isRaidlead || (this.players[i].comment.length != 0) )
				{
					HTMLString += "<div class=\"comment\">"
					
					if ( g_User.isRaidlead )
					{
						HTMLString += "<span class=\"comment_item\">";
						
						if ( this.players[i].id != 0 )
						{
							// display movement icons
							
							var displayedRole = (this.name != this.players[i].role1) ? this.players[i].role1 : this.players[i].role2;							
							HTMLString += "<img id=\"to_" + displayedRole + "_" + this.name + "_" + i + "\" onclick=\"moveSlotFromComment(this)\" class=\"comment_role\" src=\"images/classessmall/" + displayedRole + ".png\"/>";
							...
							{
							case "tank":
								HTMLString += "<div class=\"move_right\"></div>";
								break;
								
							case "dmg":
								HTMLString += "<div class=\"move_left\"></div>";
								break;
								
							default:
								if ( displayedRole == "dmg" )
									HTMLString += "<div class=\"move_right\"></div>";
								else
									HTMLString += "<div class=\"move_left\"></div>";
								break;
						...
						HTMLString += "<img id=\"to_bench_" + this.name + "_" + i + "\" onclick=\"addToBenchFromComment(this)\" class=\"comment_role\" src=\"images/classessmall/bench.png\"/>";
						HTMLString += "</span>"
					}
					
					// display comment
					
					HTMLString += "<span class=\"comment_text\">";
					
					if ( g_User.isRaidlead && (this.players[i].comment != "") )
					{
						if ( this.players[i].id == 0 )
							HTMLString += "<div class=\"comment_name\">" + this.players[i].name.substr( 0, this.players[i].name.indexOf(" ") ) + "</div>";
						else
							HTMLString += "<div class=\"comment_name\">" + this.players[i].name + "</div>";
					}
					
					HTMLString += this.players[i].comment + "</span></div>";
				}
		...
		if ( g_User.isRaidlead )
		{
			var draggableNodes = slotField.children(".slotrow").children(".occupied");
			...
			draggableNodes.each( function() {
				makeTouchable( $(this) );
			});
			
			slotField.children(".slotrow").children(".occupied").children("div")
				.dblclick( function( event ) { addToBench($(this).parent()); } );
		}
		
		slotField.children(".slotrow").children(".occupied").children(".class")
			.click( function( event ) { showAttendeeTooltip( $(this).parent(), true ); event.stopPropagation(); } );
	}
	...
	this.displayAsBench = function() 
	{
		...
		for ( i=0; i<this.players.length; ++i )
		{
			...
			HTMLString += "<img class=\"class_icon\" src=\"images/classessmall/" + this.players[i].charClass + ".png\"/>";
			...
			HTMLString += "</div>";			
			HTMLString += "<div class=\"name\">" + this.players[i].name + "</div>";
			...
			if ( g_User.isRaidlead || (this.players[i].comment.length != 0) )
			{
				...
				if ( g_User.isRaidlead )
				{
					...
					HTMLString += "<div id=\"to_" + this.players[i].role1 + "_" + this.name + "_" + i + "\" onclick=\"addToSlotFromComment(this)\" class=\"comment_role\" style=\"background-image: url(images/classessmall/" + this.players[i].role1 + ".png)\">";
					HTMLString += "<div class=\"move_up\"></div>";
					HTMLString += "</div>";
					
					if ( (this.players[i].id != 0) && (this.players[i].role1 != this.players[i].role2) )
					{
						HTMLString += "<div id=\"to_" + this.players[i].role2 + "_" + this.name + "_" + i + "\" onclick=\"addToSlotFromComment(this)\" class=\"comment_role\" style=\"background-image: url(images/classessmall/" + this.players[i].role2 + ".png)\">";
						HTMLString += "<div class=\"move_up\" style=\"left: 88px\"></div>";
						HTMLString += "</div>";
					}
				...
				HTMLString += this.players[i].comment + "</span>";				
				HTMLString += "</div>";
			}
		...
		if ( g_User.isRaidlead )
		{
			var draggableNodes = benchField.children(".slot");
		...
		for ( i=0; i<this.unavail.length; ++i )
		{
			...
			HTMLString += "<div class=\"class unavailable\">";
			HTMLString += "<img class=\"class_icon\" src=\"images/classessmall/" + this.unavail[i].charClass + ".png\"/>";
			...
			HTMLString += "</div>";
			HTMLString += "<span class=\"name unavailable shadow_default\">" + this.unavail[i].name + "</span>";
			HTMLString += "<div class=\"comment\">" + this.unavail[i].comment + "</div>";
			HTMLString += "</span>";
		}

		for ( i=0; i<this.undecided.length; ++i )
		{
			HTMLString += "<span class=\"slot\">";			
			HTMLString += "<div class=\"class undecided\">";
			HTMLString += "<img class=\"class_icon\" src=\"images/classessmall/" + this.undecided[i].charClass + ".png\"/>";
			
			HTMLString += "<div class=\"nobadge\"></div>";
			HTMLString += "</div>";
			HTMLString += "<span class=\"name undecided shadow_default\">" + this.undecided[i].name + "</span>";
			HTMLString += "</span>";
		}
		
		benchField.append( HTMLString );
		
		var Slots = benchField.children(".slot");
		
		if ( g_User.isRaidlead )
		{
			Slots.dblclick( function( event ) { addToSlot($(this)); } )
		}
						
		Slots.mouseover( function( event ) { showAttendeeTooltip( $(this), false ); } )
			
		Slots.children(".class")
			.click( function( event ) { showAttendeeTooltip( $(this).parent(), true ); event.stopPropagation(); } );
...
function adjustSlots()
{
	...
	var raidSize  = (g_User.isRaidlead) ? parseInt($("#selectsize").val()) : tankSlotObj.slots + healSlotObj.slots + dmgSlotObj.slots;
	...
	else if ( raidSize - (numTanks + numHealer) < 1 )
	{
		numHealer = raidSize - numTanks - 1;
	}
...
function displayRaid( a_XMLData )
{
	...
	if ( g_User.isRaidlead )
	{
		...
		buttonFieldHTMLString += "<option value=\"open\"" + openSelected + ">" + L("RaidOpen") + "</option>";
		buttonFieldHTMLString += "<option value=\"locked\"" + lockedSelected + ">" + L("RaidLocked") + "</option>";
		buttonFieldHTMLString += "<option value=\"canceled\"" + canceledSelected + ">" + L("RaidCanceled") + "</option>";
		buttonFieldHTMLString += "<option value=\"delete\">" + L("DeleteRaid") + "</option>";
		buttonFieldHTMLString += "</select>";
		buttonFieldHTMLString += "<div class=\"buttonfield\"><button id=\"submitchanges\">" + L("Apply") + "</button></div>";
	...
	else
	{
        ...
        HTMLString += "<div class=\"time\">" + formatTimeString(raidData.children("start").text()) + " - " + formatTimeString(raidData.children("end").text()) + "</div>";
...
function triggerRaidDelete( a_RaidId )
{
	...
	if ( g_User.isRaidlead )
	{
		confirm( L("ConfirmRaidDelete"), 
				 L("DeleteRaid"), L("Cancel"), 
				 function() {
