...
function CRaidMemberList()
{
    ...
    this.generatePlayerSlot = function( aPlayer, aClipStatus )
    {
        ...
        if ( gRaidEditMode && (aPlayer.className[0] == "___") )
    ...
    this.generateRoleList = function( aRoleId, aColumns, aRequiredSlots )
    {
        ...
        if (SetupForRole >= aRequiredSlots) {
            HTMLString += "<h3 class=\"glow_complete\">"+L(Role.name);
            HTMLString += " <span class=\"attendance_count\">"+L("Complete")+"</span></h3>";
        } else {
            HTMLString += "<h3>"+L(Role.name);
            HTMLString += " <span class=\"attendance_count\">"+SetupForRole+"/"+aRequiredSlots+"</span></h3>";
        }
        ...
        var WaitingAreBenched = ((NumActive >= aRequiredSlots) && (this.mMode != "overbook")) ||
                                (!gRaidEditMode && (this.mStage == "locked"));
...
{
    var TotalMembers = 0;
    var MaxMembers   = parseInt($("#selectsize").val(), 10);

    $.each(gGame.RaidViewOrder, function(aIndex, aRoleId) {
        TotalMembers += parseInt($("#slotCount_"+aRoleId).val(), 10);
    });

    var OldMaxMembers = TotalMembers
    TotalMembers = 0

    $.each(gGame.RaidViewOrder, function(aIndex, aRoleId) {
        var SlotElement = $("#slotCount_"+aRoleId);
        var baseValue   = parseInt(SlotElement.val(), 10);
        var RoleSlots   = Math.ceil(MaxMembers * (baseValue / OldMaxMembers));

        if (TotalMembers + RoleSlots > MaxMembers) {
            RoleSlots = MaxMembers - TotalMembers;
        }
...
function generateRaidInfo( aXHR, aAppendTo )
{
    ...
    HTMLString += "<div class=\"raidsize\"><span class=\"attends\">" + Attended + "</span>" + ((aXHR.mode != "all") ? " / "+RaidSize+" "+L("Players")  : " "+L("Attendants")) + "</div>";
...
function generateRaidSettings( aXHR )
{
    ...
    if (aXHR.type == "raid") {
        ...
        HTMLString += "<iput type=\"hidden\" id=\"raidmode\" value=\"all\"/>";
    }
...
function generateRaid( aXHR )
{
    ...
    gRaidEditMode = gUser.isRaidlead || (gUser.isPrivileged && (gUser.id == aXHR.userId));
