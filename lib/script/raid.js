        ...
        if ( gUser.isRaidlead )
        {
            ...
            Clips.children(".waitSlot, .waitSlotComment").each( function() {
                ...
                var onClickFunction = function(aEvent) {
                    ...
                    showTooltipSlackers( Icon, Player, true, onAbortFunction );
                    aEvent.stopPropagation();
                };
            ...
            Clips.children(".benchSlotComment").each( function() {
                ...
                Slot.click( function(aEvent) {
                    showTooltipSlackers( Icon, PlayerList.mPlayers[PlayerIdx], true, onAbortFunction );
                    aEvent.stopPropagation();
                });
...
function onClickAbsentSlot(aEvent, aElement, aPlayer, aUndecided)
{
    ...
    else
    {
        ...
        showTooltipSlackers( aElement, aPlayer, true, onAbortFunction);
...
function updateSlackersBadges(aPlayerList)
{
    ...
    aPlayerList.forEachPlayer( function(player) {
        ...
        if (player.status == "undecided") ++undecidedCount;
    });
    
    $("#slackers_badge").empty();
    
    if (undecidedCount > 0)
        $("#slackers_badge").append("<span style=\"color: #fc0\">"+ undecidedCount + "</span>");
        
    if ((undecidedCount > 0) && (absentCount > 0))
        $("#slackers_badge").append("/");
    ...
    $("#absentList > .clip").children(".benchSlotComment, .benchSlot").each( function() {
        ...
        else
        {
            Element.mouseover( function() {
                showTooltipSlackers( Element, Player, false, null );
                aEvent.stopPropagation();
            });
        }
    });
...
function generateRaidSettings( aXHR )
{
    ...
    $.each(aXHR.slots, function(index, value) {
        if ( index < gRoleIds.length )
        {
            HTMLString += "<div><div class=\"settingLabel\">"+L("RequiredForRole")+" \""+gRoleNames[gRoleIdents[index]]+"\"</div>";
